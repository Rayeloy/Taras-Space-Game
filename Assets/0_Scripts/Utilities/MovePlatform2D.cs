...
public class MovePlatform2D : MonoBehaviour
{
    ...
    private void Update()
    {
        if (executionMode == UpdateMode.Update)
        {
    ...
    {
        if ((executionMode == UpdateMode.FixedUpdate /*|| executionMode == UpdateMode.FixedUpdateRB*/))
        {
            ...
            switch (executionMode)
            {
                case UpdateMode.FixedUpdate:
                    ...
                    {
                        currentSideAmp += sidewaysSpeed * Time.deltaTime * sideSentido;
                        Vector3 newPos = new Vector3(originPos.x + currentSideAmp, myRb.position.y);
